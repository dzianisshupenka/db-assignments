    ...
    const result = await db.collection('employees').aggregate([
        ...
        {$unwind: {path: "$Reports", preserveNullAndEmptyArrays: true}},
        {
            $project: {
                ...
                ReportsTo: { $ifNull: [{ $concat: ["$Reports.FirstName", " ", "$Reports.LastName"]}, "-"]}
            }
        },
        {$sort: {EmployeeID: 1}}
    ]).toArray();
...
async function task_1_13(db) {
    const CurrentProducts = await db.collection('products').find().count();
    const result = await db.collection('products').aggregate([
        {
            $match: {Discontinued: 1} 
        },{
            ...
            $addFields: {TotalOfCurrentProducts: CurrentProducts}
        },{
            $project: {
                _id: 0,
...
async function task_1_16(db) {
    const result = await db.collection('orders').aggregate([
        {
            $match: {ShipPostalCode: {$exists: true}}
        },{
            $project: {
                _id: 0,
                OrderID: 1,
    ...
    const result = await db.collection('orders').aggregate([
        {
            ...
            $project: {
                _id: 0,
                Day: {$dayOfYear: { $dateFromString: { dateString: '$OrderDate' } }},
            ...
            $project: {
                _id: 0,
                "Total Number of Orders": 1,
                "Order Date": { $dateToString: { format: "%Y-%m-%d", date: "$OrderDate" } }
            }
        },{
            $sort: {"Order Date": 1}
        }
    ...
    const result = await db.collection('order-details').aggregate([
        {
            $group: {
                _id: "$OrderID",
                OrdersAmount: {$sum: {$multiply: ["$UnitPrice", "$Quantity"]}}
            }
        },{
            $lookup: {
                from: "orders",
            ...
            $group: {
                ...
                TotalOrdersAmount: {$sum: "$OrdersAmount"}
            }
        },{
            $match: {
    ...
    const result = await db.collection('order-details').aggregate([
        {
            ...
            $group: {
                ...
                EmployeeOrdersAmount: {$sum: "$OrdersAmount"}
            }
        },{
            $sort:{EmployeeOrdersAmount: -1}
        },{$limit: 1},{
            $lookup: {
                from: "employees",
                localField: "_id",
    ...
    const result = await db.collection('order-details').aggregate([
        {
            ...
            $project: {
                _id: 0,
                "OrderID": "$_id",
                "Maximum Purchase Amount, $": "$OrdersAmount"
            }
        }
    ]).toArray();
    return result;
